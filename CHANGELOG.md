...
- **SQL Server: fix table preview for connections without a default database.** The tree lists databases (drilling into each one's `dbo` schema), but the preview emitted `[dbname].[table]`, which T-SQL resolves as _schema_.table inside `master`. It now emits a native three-part name — `[dbname].[dbo].[table]`.
