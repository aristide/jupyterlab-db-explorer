...
def _getSQL_engine(dbid, db, usedb=None):
    ...
    elif db['db_type'] == DB_TRINO:  # Trino
        db_port = db['db_port'] if 'db_port' in db else 8080
        sqlstr = f"trino://{db_user}:{db_pass}@{db_host}:{db_port}/{db_name}"
    elif db['db_type'] == DB_STARROCKS:  # StarRocks
        db_port = db['db_port'] if 'db_port' in db else 9030
        sqlstr = f"mysql+pymysql://{db_user}:{db_pass}@{db_host}:{db_port}/{db_name}"
    else:
