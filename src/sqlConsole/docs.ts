...
const GENERIC: Record<string, ISqlDoc> = {
  ...
  DELETE: {
    summary:
      'Remove rows matching a condition. Without WHERE, removes ALL rows.',
  ...
  'LEFT JOIN': {
    summary:
      'Return all rows from the left table, with matches from the right (NULL when no match).',
    example:
      'SELECT u.name, o.id\nFROM users u\nLEFT JOIN orders o ON o.user_id = u.id;'
  },
  GROUP: {
    summary:
      'GROUP BY collapses rows that share values into a single output row per group.',
    signature: 'SELECT col, AGG(...) FROM t GROUP BY col',
    example: 'SELECT country, COUNT(*) FROM users GROUP BY country;'
  },
  HAVING: {
    summary:
      'Filter groups produced by GROUP BY (post-aggregation, unlike WHERE).',
    example:
      'SELECT country, COUNT(*) AS n\nFROM users\nGROUP BY country\nHAVING COUNT(*) > 100;'
  },
  ORDER: {
    summary:
      'Sort rows by one or more columns. Direction: ASC (default) or DESC.',
  ...
  COUNT: {
    summary:
      'Count rows. COUNT(*) counts all; COUNT(col) skips NULLs; COUNT(DISTINCT col) counts unique.',
  ...
  UNION: {
    summary:
      'Combine two result sets, removing duplicates. UNION ALL keeps duplicates (faster).',
...
const PG_DOCS: Record<string, ISqlDoc> = {
  ...
  RETURNING: {
    summary:
      'Postgres-only: return columns from the row that was INSERT/UPDATE/DELETE-d.',
...
const MYSQL_DOCS: Record<string, ISqlDoc> = {
  ...
  ON_DUPLICATE_KEY_UPDATE: {
    summary:
      'Upsert-style: UPDATE the matched row if INSERT hits a unique-key conflict.',
    example:
...
const TRINO_DOCS: Record<string, ISqlDoc> = {
  ...
  LEAST: {
    summary: 'Smallest of the given values (NULLs ignored unless all are NULL).'
  },

  // ── Date & time ──────────────────────────────────────────────────────
  DATE_TRUNC: {
    summary:
      'Truncate timestamp to unit (second, minute, hour, day, week, month, quarter, year).',
  ...
  DATE_FORMAT: {
    summary:
      'Format a timestamp using MySQL-style specifiers (%Y, %m, %d, %H, %i, %s).',
    example: "SELECT DATE_FORMAT(NOW(), '%Y-%m-%d %H:%i:%s');"
  },
  DATE_PARSE: {
  ...
  AT_TIMEZONE: {
    ...
    example:
      "SELECT timestamp '2024-01-01 00:00:00 UTC' AT TIME ZONE 'America/New_York';"
  },
  LAST_DAY_OF_MONTH: {
  ...
  APPROX_DISTINCT: {
    summary:
      'HyperLogLog-based approximate count distinct. Much cheaper than COUNT(DISTINCT).',
  ...
  ARBITRARY: {
    summary:
      'Return an arbitrary non-NULL value from the group (faster than MIN/MAX when you just need any).',
  ...
  MAP_AGG: {
    ...
    example:
      'SELECT user_id, MAP_AGG(metric, value) FROM stats GROUP BY user_id;'
  },
  MULTIMAP_AGG: {
    summary:
      'Like MAP_AGG but the values for a key are collected into an array.',
  ...
  MAX_BY: {
    ...
    example:
      "SELECT MAX_BY(name, score) FROM players;   -- highest scorer's name"
  },
  MIN_BY: {
    summary: 'Return `x` at the row where `y` is the minimum.',
    signature: 'MIN_BY(x, y [, n])'
  },
  COUNT_IF: {
  ...
  BOOL_AND: {
    summary: 'Logical AND across a group (TRUE only if all rows are TRUE).'
  },
  BOOL_OR: { summary: 'Logical OR across a group (TRUE if any row is TRUE).' },
  EVERY: { summary: 'Synonym for BOOL_AND.' },
  GEOMETRIC_MEAN: {
    summary: 'Geometric mean of non-negative values in the group.'
  },

  // ── Arrays ───────────────────────────────────────────────────────────
  UNNEST: {
    summary: 'Expand an array (or map) into rows. Use with CROSS JOIN.',
    example: 'SELECT u.id, t\nFROM users u\nCROSS JOIN UNNEST(u.tags) AS x(t);'
  },
  CARDINALITY: {
    summary:
      'Number of elements in an array or map (use this instead of LENGTH for those types).',
    example: 'SELECT CARDINALITY(tags) FROM users;'
  },
  ELEMENT_AT: {
    summary:
      'Get the element at the given index (1-based) for arrays, or by key for maps. Returns NULL if missing.',
    example:
      "SELECT ELEMENT_AT(tags, 1), ELEMENT_AT(props, 'color') FROM items;"
  },
  CONTAINS: {
  ...
  ARRAY_SORT: {
    summary:
      'Sort array elements ascending. Pass a comparator lambda for custom order.',
    example: 'SELECT ARRAY_SORT(ARRAY[3,1,2]);   -- [1,2,3]'
  },
  ARRAY_REMOVE: {
    summary: 'Remove all occurrences of a value from the array.'
  },
  FILTER: {
  ...
  REDUCE: {
    summary:
      'Fold an array down to a single value via init + combine + finish.',
  ...
  SEQUENCE: {
    ...
    example:
      "SELECT SEQUENCE(DATE '2024-01-01', DATE '2024-01-31', INTERVAL '1' DAY);"
  },
  SLICE: {
  ...
  MAP: {
    summary:
      'Construct a map from two arrays (keys + values), or as map_from_entries.',
  ...
  MAP_CONCAT: {
    summary: 'Merge multiple maps. Right-most wins on duplicate keys.'
  },
  MAP_FILTER: {
  ...
  MAP_ZIP_WITH: {
    ...
    example:
      'SELECT MAP_ZIP_WITH(a, b, (k, va, vb) -> coalesce(va,0) + coalesce(vb,0));'
  },
  ...
  STRPOS: {
    summary:
      '1-based position of the first occurrence of a substring (0 if not found).',
    example: "SELECT STRPOS('hello world', 'world');   -- 7"
  },
  CONCAT: { summary: 'Concatenate strings or arrays.' },
  POSITION: {
    summary: 'SQL-standard alternative to STRPOS: POSITION(needle IN haystack).'
  },
  LISTAGG: {
    summary: 'String aggregation with separator and overflow handling.',
    signature:
      'LISTAGG(expr, sep [ON OVERFLOW TRUNCATE]) WITHIN GROUP (ORDER BY ...)',
    example:
  ...
  JSON_EXTRACT_SCALAR: {
    summary:
      'Like JSON_EXTRACT but always returns VARCHAR (not JSON). Best for atom-typed leaves.',
  ...
  LAG: {
    ...
    example:
      'SELECT ts, value, LAG(value) OVER (PARTITION BY user_id ORDER BY ts) AS prev FROM events;'
  },
  LEAD: {
    summary: 'Value from a following row in the partition (mirror of LAG).'
  },
  ROW_NUMBER: {
  ...
  RANK: {
    summary: 'Rank within a window — ties get the same rank, leaving gaps.'
  },
  DENSE_RANK: {
    summary:
      'Like RANK but ties share a rank and the next rank is +1 (no gaps).'
  },
  NTILE: {
    summary: 'Bucket rows of the partition into N equally-sized groups.'
  },
  FIRST_VALUE: { summary: 'First value in the window frame.' },
  LAST_VALUE: {
    summary: 'Last value in the window frame (mind the default frame!).'
  },
  NTH_VALUE: { summary: 'The Nth value (1-based) in the window frame.' },
  CUME_DIST: {
    summary:
      'Cumulative distribution: fraction of rows ≤ current row in the window.'
  },
  PERCENT_RANK: {
    summary: '(rank - 1) / (partition_rows - 1). Useful for percentile bands.'
  },
  ...
  VALUES: {
    summary:
      'Inline literal table — useful for ad-hoc data or for joining against a fixed set.',
    example: "SELECT * FROM (VALUES (1, 'a'), (2, 'b')) AS t(id, label);"
  },
  GROUPING: {
    summary:
      "Returns 1 if the grouping set didn't group by the listed column (use in ROLLUP/CUBE output).",
    example:
      'SELECT GROUPING(country), COUNT(*) FROM users GROUP BY ROLLUP(country);'
  },
  ROLLUP: {
  ...
  TABLESAMPLE: {
    summary:
      'Read a random sample of the table. Two methods: BERNOULLI (row-level) and SYSTEM (block-level).',
    example: 'SELECT * FROM events TABLESAMPLE BERNOULLI (1);   -- 1% sample'
  },
  EXPLAIN: {
    summary:
      'Show the query plan. EXPLAIN ANALYZE actually runs the query and reports timings.',
    example:
      'EXPLAIN ANALYZE SELECT COUNT(*) FROM events WHERE day = current_date;'
  },
  SHOW: {
    summary:
      'Metadata queries: SHOW CATALOGS / SCHEMAS / TABLES / COLUMNS / FUNCTIONS / SESSION.',
  ...
  UUID: {
    summary: 'Return a random RFC 4122 v4 UUID.',
    example: 'SELECT UUID();'
  },
  RAND: { summary: 'Random double in [0, 1).' },
  RANDOM: {
    summary: 'Synonym for RAND. RANDOM(n) returns a random int in [0, n).'
  }
};
...
const HIVE_DOCS: Record<string, ISqlDoc> = {
  EXPLODE: {
    summary:
      'Hive UDTF: expand an array column into one row per element. Use with LATERAL VIEW.',
    example:
      'SELECT u.id, tag\nFROM users u\nLATERAL VIEW EXPLODE(u.tags) t AS tag;'
  },
  POSEXPLODE: {
    summary: 'Like EXPLODE but also returns a position column.',
    example: 'LATERAL VIEW POSEXPLODE(arr) t AS pos, val'
  },
  COLLECT_SET: {
  ...
  GET_JSON_OBJECT: {
    summary:
      'Extract a field from a JSON string using a JSONPath-like expression.',
  ...
  'LATERAL VIEW': {
    summary:
      'Apply a UDTF (EXPLODE, POSEXPLODE, ...) and join its rows to the outer row.',
...
const STARROCKS_DOCS: Record<string, ISqlDoc> = {
  ...
  TO_BITMAP: {
    summary:
      'Convert an unsigned 64-bit integer column into a bitmap value. The base building block for COUNT DISTINCT acceleration.',
    signature: 'TO_BITMAP(int_value)',
    example:
      'SELECT BITMAP_UNION(TO_BITMAP(user_id))\nFROM events\nGROUP BY day;'
  },
  BITMAP_UNION: {
    summary:
      'Aggregate function: merge BITMAP values into a single bitmap (set-union).',
    example:
      'SELECT day, BITMAP_UNION(user_bitmap) FROM daily_active GROUP BY day;'
  },
  BITMAP_UNION_COUNT: {
    summary:
      'Aggregate distinct count via bitmap union. Equivalent to BITMAP_COUNT(BITMAP_UNION(x)) but faster in one shot.',
    example:
      'SELECT day, BITMAP_UNION_COUNT(user_bitmap) AS dau FROM daily_active GROUP BY day;'
  },
  BITMAP_INTERSECT: {
    summary:
      'Aggregate function: bitmap intersection — IDs present in ALL groups. Useful for retention/cohort analysis.',
    example:
      "SELECT BITMAP_INTERSECT(user_bitmap) FROM daily_active WHERE day IN ('2024-01-01', '2024-01-02');"
  },
  BITMAP_AND: {
  ...
  BITMAP_AND_COUNT: {
    summary:
      'Count of set bits in BITMAP_AND(a, b). Cheaper than building the result.'
  },
  ...
  BITMAP_COUNT: {
    summary: 'Number of set bits (= distinct integers) in the bitmap.'
  },
  BITMAP_EMPTY: {
    summary: 'Construct an empty bitmap literal — useful for default values.'
  },
  BITMAP_FROM_STRING: {
  ...
  BITMAP_TO_STRING: {
    summary:
      'Render a bitmap as a comma-separated integer string (debugging / export).',
    example: 'SELECT BITMAP_TO_STRING(user_bitmap) FROM daily_active LIMIT 1;'
  },
  BITMAP_HASH: {
    summary:
      'Hash an arbitrary string column into a 64-bit int suitable for TO_BITMAP. Use when your unique IDs are strings.',
    example:
      'SELECT BITMAP_UNION(TO_BITMAP(BITMAP_HASH(email))) FROM users GROUP BY country;'
  },

  // ── HLL (HyperLogLog) — alternative for approximate distinct ────────
  HLL_UNION: {
    summary:
      'Aggregate function: union HLL values into one. Pair with HLL_HASH / HLL_CARDINALITY.',
    example: 'SELECT HLL_UNION(hll_col) FROM daily_users GROUP BY week;'
  },
  HLL_UNION_AGG: {
    summary:
      'Approximate distinct count via HLL — like APPROX_COUNT_DISTINCT but on the HLL type.',
    example: 'SELECT HLL_UNION_AGG(hll_col) FROM daily_users;'
  },
  HLL_HASH: {
    summary:
      "Hash a value into an HLL register. Use when you didn't ingest an HLL column directly.",
    example: 'SELECT HLL_UNION_AGG(HLL_HASH(session_id)) FROM events;'
  },
  HLL_CARDINALITY: {
    summary: 'Approximate distinct count of a single HLL value.'
  },
  ...
  PERCENTILE_UNION: {
    summary:
      'Merge pre-aggregated PERCENTILE values (used with materialized-view rollups).',
    example: 'SELECT PERCENTILE_UNION(p_state) FROM rollup_p95;'
  },
  PERCENTILE_CONT: {
    summary:
      'Continuous (interpolated) percentile — exact, but slower than PERCENTILE_APPROX.',
    example: 'SELECT PERCENTILE_CONT(price, 0.5) FROM listings;   -- median'
  },
  PERCENTILE_DISC: {
    summary:
      'Discrete percentile — picks an actual value in the column (no interpolation).'
  },
  NDV: {
    summary:
      'Approximate distinct count (NDV = Number of Distinct Values). Backed by HLL — same engine as HLL_UNION_AGG.',
    example: 'SELECT NDV(user_id) FROM events;'
  },
  MULTI_DISTINCT_COUNT: {
    summary:
      'Faster COUNT(DISTINCT col) when you actually need exact results — uses a multi-pass plan StarRocks optimizes.',
    example: 'SELECT MULTI_DISTINCT_COUNT(user_id) FROM events;'
  },
  MULTI_DISTINCT_SUM: {
    summary:
      'Sum-of-distinct, same fast-path treatment as MULTI_DISTINCT_COUNT.'
  },
  MAX_BY: {
    summary: 'Return `x` at the row where `y` is the maximum.',
  ...
  WINDOW_FUNNEL: {
    summary:
      'Funnel analysis: per partition, returns the deepest ordered step matched within a time window.',
  ...
  RETENTION: {
    summary:
      'Cohort retention: returns an array of 0/1 flags — first element marks the cohort event, the rest mark return events.',
    example:
      "SELECT RETENTION(event = 'signup', event = 'login') FROM events GROUP BY user_id;"
  },
  ...
  ARRAY_LENGTH: {
    summary: 'Number of elements in the array (use this, not LENGTH).'
  },
  ARRAY_CONTAINS: {
  ...
  ARRAY_POSITION: {
    summary: '1-based position of the value in the array, or 0 if absent.'
  },
  ARRAY_REMOVE: {
    summary: 'Remove all occurrences of a value from the array.'
  },
  ARRAY_SLICE: {
    summary: 'Sub-array: ARRAY_SLICE(array, offset, length). 1-based.'
  },
  ARRAY_SORT: { summary: 'Sort the array ascending.' },
  ARRAYS_OVERLAP: {
    summary: 'TRUE if two arrays share at least one common element.'
  },
  ...
  JSON_LENGTH: {
    summary: 'Length of the JSON array/object at the given path.'
  },
  JSON_OBJECT: {
  ...
  JSON_TYPE: {
    summary:
      'Return the JSON type at the given path: object, array, string, number, bool, null.',
  ...
  DATE_TRUNC: {
    summary:
      'Truncate a datetime to the given unit. StarRocks unit names follow MySQL (day, week, month, …).',
  ...
  DATE_DIFF: {
    summary:
      'Days between two dates (MySQL semantics: a − b, can be negative).',
  ...
  DATE_SLICE: {
    summary:
      'Bucket a datetime to a fixed-size window. Useful for time-bucketed grouping.',
    signature: 'DATE_SLICE(datetime, interval, unit [, boundary])',
    example:
      "SELECT DATE_SLICE(ts, 5, 'minute') AS bucket, COUNT(*) FROM events GROUP BY bucket;"
  },
  ...
  UNIX_TIMESTAMP: {
    summary: 'Same as TO_UNIX_TIMESTAMP, or NOW() as epoch when no arg.'
  },
  ...
  DUPLICATE: {
    summary:
      'DUPLICATE KEY (cols) — table model where rows are appended verbatim. Fastest writes, no de-dup.',
    example:
      'CREATE TABLE events (\n  ts DATETIME, user_id BIGINT, event VARCHAR(64)\n)\nDUPLICATE KEY(ts, user_id)\nDISTRIBUTED BY HASH(user_id) BUCKETS 32;'
  },
  AGGREGATE: {
    summary:
      'AGGREGATE KEY (cols) — table model that pre-aggregates value columns by their declared aggregator (SUM, REPLACE, BITMAP_UNION, HLL_UNION, …).',
    example:
      'CREATE TABLE daily_users (\n  day DATE,\n  user_bitmap BITMAP BITMAP_UNION\n)\nAGGREGATE KEY(day)\nDISTRIBUTED BY HASH(day) BUCKETS 8;'
  },
  UNIQUE: {
    summary:
      'UNIQUE KEY (cols) — upsert-like model: later rows with the same key overwrite earlier ones.',
    example:
      'CREATE TABLE user_profile (\n  user_id BIGINT, name VARCHAR(64), updated DATETIME\n)\nUNIQUE KEY(user_id)\nDISTRIBUTED BY HASH(user_id) BUCKETS 16;'
  },
  DISTRIBUTED: {
    summary:
      'DISTRIBUTED BY HASH(...) BUCKETS N — controls how rows are sharded across tablets.',
    example: 'DISTRIBUTED BY HASH(user_id) BUCKETS 32'
  },
  PARTITION: {
    summary:
      'PARTITION BY RANGE(...) — define data partitions; dynamic partitions auto-create over time via PROPERTIES.',
    example:
      "PARTITION BY RANGE(day) (\n  PARTITION p202401 VALUES LESS THAN ('2024-02-01'),\n  PARTITION p202402 VALUES LESS THAN ('2024-03-01')\n)"
  },
  PROPERTIES: {
    summary:
      'Table options block: replication, storage, dynamic-partition rules, etc.',
    example:
      "PROPERTIES (\n  'replication_num' = '3',\n  'dynamic_partition.enable' = 'true',\n  'dynamic_partition.time_unit' = 'DAY',\n  'dynamic_partition.start' = '-30',\n  'dynamic_partition.end' = '3'\n)"
  },
  ROLLUP: {
    summary:
      'Materialized rollup index on a subset of columns — speeds up specific query shapes.',
    example:
      'ALTER TABLE events ADD ROLLUP r_country_day(day, country, user_bitmap);'
  },
  MATERIALIZED: {
    summary:
      'MATERIALIZED VIEW — a query result kept fresh by StarRocks. Used for OLAP acceleration.',
    example:
      'CREATE MATERIALIZED VIEW mv_dau AS\nSELECT day, BITMAP_UNION(TO_BITMAP(user_id)) AS user_bitmap\nFROM events GROUP BY day;'
  },
  BROKER: {
    summary:
      'BROKER LOAD — ingest external data via a configured broker (S3/HDFS/etc.).',
    example:
      "LOAD LABEL events_2024 (DATA INFILE('hdfs://…/events.parq') INTO TABLE events) WITH BROKER 'broker0';"
  },
  ROUTINE: {
  ...
  STREAM: {
    summary:
      'STREAM LOAD — push data via HTTP. Use the curl-based loader (no SQL syntax in the editor).'
  },
  ...
  NULLIF: {
    summary: 'Return NULL if the two args are equal, otherwise the first.'
  },
  IF: {
...
const SQLITE_DOCS: Record<string, ISqlDoc> = {
  ...
  GLOB: {
    summary:
      'Pattern match using Unix shell glob syntax (*, ?). Case-sensitive (unlike LIKE).',
...
const MSSQL_DOCS: Record<string, ISqlDoc> = {
  TOP: {
    summary:
      'SQL Server: limit rows returned (placed right after SELECT, unlike LIMIT).',
    example: 'SELECT TOP 10 * FROM events ORDER BY ts DESC;'
  },
  OFFSET: {
    summary:
      'Skip N rows. SQL Server requires ORDER BY when using OFFSET ... FETCH.',
    example:
      'SELECT * FROM events ORDER BY ts OFFSET 100 ROWS FETCH NEXT 10 ROWS ONLY;'
  },
  ISNULL: {
    summary:
      'SQL Server: return the second argument if the first is NULL. (Not standard COALESCE.)',
...
const ORACLE_DOCS: Record<string, ISqlDoc> = {
  ROWNUM: {
    summary:
      'Pseudo-column: sequence number Oracle assigns to a row before ORDER BY.',
    example: 'SELECT * FROM users WHERE ROWNUM <= 10;'
  },
  CONNECT: {
    summary: 'CONNECT BY: Oracle hierarchical query.',
    example:
      'SELECT name FROM employees START WITH manager_id IS NULL CONNECT BY PRIOR id = manager_id;'
  },
